 ...
 */
export function minimizeForLLM(article: ArticleResult | null, actions: ScannedAction[]): string {
    const parts: string[] = [];
    ...
    if (limited.length > 0) {
        const actionLines = limited.map((a) => {
            let line = `${a.id} [${a.tag}] ${a.text.slice(0, MAX_ACTION_LABEL_CHARS).trim()}`;
            if (a.tag === "select" && a.options?.length) {
                const opts = a.options.map((o) => `${o.value}:${o.label}`).join(", ");
                line += ` | ${opts}`;
            }
            return line;
        });
